module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if(client.blacklistedUsers.has(user.id)) {
      ...
      if(settings.owners.has(X.operator) && !settings.owners.has(message.author.id)) return message.reply(`**${user} was blacklisted by a Centauri Bot owner, meaning no one except Centauri Bot owners can remove him.`)
    }
